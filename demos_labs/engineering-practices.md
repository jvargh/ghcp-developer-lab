...
**Validation 1: Code completions are suppressed in excluded files**

1.  In VS Code, open a file inside `node_modules/`. Use **File → Open File** and navigate to:
2.  Place your cursor at the end of any line and press **Enter** to create a new line.
3.  Type `"description":` and pause for 1–2 seconds.
4.  **Expected:** No ghost text (inline completion) appears. Copilot does not offer suggestions inside excluded paths.
5.  Now open `src/app/page.tsx`, go to the bottom of the file, and type `// add a` on a new line.
6.  **Expected:** Copilot _does_ offer a completion here, confirming that completions work normally in non-excluded files.
...
**Validation 3: Excluded files don't influence code suggestions in other files**
